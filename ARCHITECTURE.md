...
- The pinned `karcher-home==0.5.1` `Product` enum does not include RVM4. The adapter patches `karcher.consts.Product` and `karcher.device.Product` at import time with `adapter._PatchedProduct` so the RVM4 product ID resolves in `Device.__init__` without crashing account-wide discovery.
