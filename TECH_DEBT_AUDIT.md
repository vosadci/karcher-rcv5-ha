...
6. ~~**`sn`, `product_id`, `nickname` stored redundantly in config entry data** — only `device_id` is used by `async_setup_entry`; the rest are dead weight that need migration-safe handling to remove.~~ **FIXED**
...
8. ~~**Room select matches by name, not ID** — two rooms with the same name silently break selection.~~ **FIXED**
...
| F006 | Architectural decay | [config_flow.py:216–218](custom_components/karcher_home_robots/config_flow.py#L216) | Medium | M | **FIXED** ~~`sn`, `product_id`, `nickname` stored redundantly in config entry data.~~ Removed from `_create_entry`; added v2→v3 migration (`_migrate_v2_to_v3`) that strips them from existing entries. `VERSION` bumped to 3. | Resolved. |
...
| F008 | Error handling | [select.py:111–113](custom_components/karcher_home_robots/select.py#L111) | Medium | S | **FIXED** ~~`async_select_option` matched by name; duplicate room names broke selection.~~ Options now formatted as `"{room_id}:{name}"`; lookup parses the ID prefix. | Resolved. |
...
| F012 | Consistency | [coordinator.py:22](custom_components/karcher_home_robots/coordinator.py#L22) | Low | S | **FIXED** ~~Two monotonic clocks with no inline explanation.~~ Added comment on `import time` line: `time.monotonic()` for outage duration (survives loop suspension); `hass.loop.time()` for update ordering. | Resolved. |
| F013 | Type/contract | [vacuum.py:85](custom_components/karcher_home_robots/vacuum.py#L85) | Low | S | **FIXED** ~~`coordinator._device` accessed from entity files.~~ Added `coordinator.device` public property; all entity files updated. | Resolved. |
| F014 | Documentation | [adapter.py:323](custom_components/karcher_home_robots/adapter.py#L323) | Low | S | **FIXED** ~~`get_rooms` bare-except comment only said "no map?".~~ Comment now distinguishes `KarcherHomeException` path (re-raised above) from unexpected errors (protobuf decode failures etc). | Resolved. |
...
### 5. F008 — Room select name-collision bug (select.py:111–113) — **FIXED**
...
- [x] **F013** — Add `coordinator.device` public property to `KarcherCoordinator`; remove all `coordinator._device` access from entity files (8 sites)
- [x] **F012** — Add 2-line clarifying comment distinguishing `time.monotonic()` vs `hass.loop.time()` in `coordinator.py`
- [x] **F014** — Improve `get_rooms` exception comment to distinguish map-not-found vs unexpected error paths
...
1. ~~**`sn`, `product_id`, `nickname` in config entry data (F006)**: Are these stored for future use?~~ **RESOLVED** — no future use planned; removed via v2→v3 migration.
