...
| Support for Kärcher models other than RCV5 | Setup no longer crashes for RCV3/RCF3 (the other members of `karcher.consts.Product` — the library's own enum misnames the latter `RCF5`; the vendor app's decompiled source confirms it's actually RCF3, product ID `1599715149861306368`) and the device registry shows their real model instead of a hardcoded "RCV5" — see `adapter._model_name()`/`_MODEL_NAMES`. Protocol *semantics* remain untested for them: `const.py`'s `work_mode`/fault-code tables, `select.py`'s mop-attachment heuristic, and map parsing were reverse-engineered from RCV5 traffic only, and may not hold. A model outside the enum entirely still fails setup for the whole account (`UnsupportedDeviceError`, ARCHITECTURE.md — Error taxonomy), just with a clear cause instead of a raw crash |
