...
| Version pin: `==0.5.1` | Hard | Pinned exactly in both `pyproject.toml` and `manifest.json`; the library is the entire cloud protocol and its last release is from October 2023. A bump requires a hardware pass. Fork/vendor triggers are in `LIBRARY.md` |
...
| REST certificate pinning | Hard | `consts.py` holds a hardcoded SHA-256 thumbprint applied via `aiohttp.Fingerprint` on every request. A server-certificate rotation breaks all REST calls and only a library release can fix it (`LIBRARY.md` trigger 1) |
| MQTT certificate verification absent | Hard | `karcher/mqtt.py` connects with `cert_reqs=ssl.CERT_NONE` + `tls_insecure_set(True)`. Encrypted but unauthenticated; cannot be fixed without a patch or a fork. Disclosed in the README security section |
| Internal API access required | Hard | The library does not expose a sufficient public API; `_mqtt`, `_mqtt.on_message`, `_update_device_properties`, `_device_props`, `_base_url`, `_mqtt_url`, `_country`, `_language`, `net_stauts` and `_download` are accessed directly. Confined to `adapter.py`, each call site carrying a `# private-api:` reason; the allowlist is enforced by `tests/tools/check_imports.py` and mirrored in `ARCHITECTURE.md` |
| `net_stauts` typo bug | Hard | `DeviceProperties.net_stauts` (misspelled) raises `AttributeError` on every property update that includes nested objects. The library updates its cache *before* the error fires, so the work-around suppresses the `AttributeError` and reads the cache (`adapter.py`, `contextlib.suppress`) rather than stripping the payload |
...
| Constraint | Type | Detail |
|---|---|---|
| Minimum HA version: 2026.9.0 | Soft | Declared in `hacs.json`. Set to the version against which the integration was developed and tested. Can be lowered if tested on earlier versions |
...
| Support for Kärcher models other than RCV5 | Every robot the cloud returns gets the full entity set and registers under its real model name; an ID outside `_model_profile.PROFILES` registers under the raw product ID and no longer affects any other robot on the account (`adapter._LenientProduct._missing_`). Protocol *semantics* remain untested beyond the RCV 5: `const.py`'s `work_mode`/fault-code tables, `select.py`'s mop-attachment heuristic, and map parsing were reverse-engineered from RCV 5 traffic only, and Kärcher's own backend puts the RCV 5 on a property schema shared with no other product (`doc/PROTOCOL.md` §16.5), so they may not hold. `_model_profile.SupportTier` records how much evidence each model has; it never gates entities |
